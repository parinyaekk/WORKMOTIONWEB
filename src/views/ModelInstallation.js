...
class ModelInstallation extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            menu_name: 'Manage Model Installation',
            Permission: "",
    ...
    async BindingDataEditPopup(id) {
        ...
        await axios.post(`${APIUrl}Installation/GetModelInstallationById`, temp)
            .then((response) => {
                if (response.data.status == 0) {
    ...
    async GetDataModel(Page, PerPage, Search, StartDate, EndDate) {
        ...
        await axios.post(`${APIUrl}Installation/GetAllModelInstallation`, temp)
            .then(response => {
                if (response.data.status == 0) {
    ...
    async InActiveDataModel() {
        ...
        await axios.post(`${APIUrl}Installation/InActiveModelInstallationById?ID=` + ID)
            .then(response => {
                if (response.data.status == 0) {
    ...
    async SaveFile(file) {
        ...
        await axios.post(`${APIUrl}Installation/UploadFileModel`, data,
        {
            headers: {
    ...
    AddDataModel(){
        ...
        axios.post(`${APIUrl}Installation/SaveAddModelInstallation`, Tempdata)
        .then((response) => {
            if (response.data.status == 0) {
                alert(response.data.message);
                window.location.reload();
    ...
    SaveEditDataModel(){
        ...
        axios.post(`${APIUrl}Installation/SaveEditModelInstallation`, Tempdata)
        .then((response) => {
            if (response.data.status == 0) {
                alert(response.data.message);
                window.location.reload();
                    ...
                    <div class="container">
                        <div class="page-header-content pt-4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                Manage Model Installation
                            </h1>
                                    <div class="page-header-subtitle">ข้อมูลรุ่นการติดตั้ง</div>
                                </div>
                            </div>
                        </div>
                    </div>
