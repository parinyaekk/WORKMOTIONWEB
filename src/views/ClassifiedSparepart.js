...
class ClassifiedSparepart extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            menu_name: 'Manage Classified SparePart',
            Permission: "",
    ...
    async BindingDataEditPopup(id) {
        ...
        await axios.post(`${APIUrl}SparePart/GetClassifiedSparePartById`, temp)
            .then((response) => {
                if (response.data.status == 0) {
    ...
    async GetOptionModel() {
        await axios.post(`${APIUrl}SparePart/GetOptionModelSparePart`)
            .then((response) => {
                if (response.data.status == 0) {
    ...
    async GetOptionClassified() {
        await axios.post(`${APIUrl}SparePart/GetOptionClassifiedSparePart`)
            .then((response) => {
                if (response.data.status == 0) {
    ...
    async GetDataClassified(Page, PerPage, Search, StartDate, EndDate) {
        ...
        await axios.post(`${APIUrl}Sparepart/GetAllClassifiedSparePart`, temp)
            .then(response => {
                if (response.data.status == 0) {
    ...
    async InActiveDataClassified() {
        ...
        await axios.post(`${APIUrl}SparePart/InActiveClassifiedSparePartById?ID=` + ID)
            .then(response => {
                if (response.data.status == 0) {
    ...
    AddDataClassified() {
        ...
        axios.post(`${APIUrl}SparePart/SaveAddClassifiedSparePart`, Tempdata)
            .then((response) => {
                if (response.data.status == 0) {
                    alert(response.data.message);
                    window.location.reload();
    ...
    SaveEditDataClassified() {
        ...
        axios.post(`${APIUrl}SparePart/SaveEditClassifiedSparePart`, Tempdata)
            .then((response) => {
                if (response.data.status == 0) {
                    alert(response.data.message);
                    window.location.reload();
                            ...
                            <div class="row align-items-center justify-content-between">
                                <div class="col-auto mt-4">
                                    <h1 class="page-header-title">
                                        ...
                                        Manage Classified SparePart
                                    </h1>
                                    <div class="page-header-subtitle">ข้อมูลประเภทอะไหล่</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default ClassifiedSparepart;
