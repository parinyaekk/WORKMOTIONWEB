...
class CustomerOpenID extends React.Component {
    ...
    async componentWillMount() {
        ...
        await this.GetDataCustomerIgnorePerPage(this.state.Page, this.state.PerPage, this.state.Search, this.state.StartDateSearch, this.state.EndDateSearch);
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            menu_name: 'Manage Membership Ignore',
            Permission: "",
            StartDateSearch: '',
            EndDateSearch: '',
            CustomerData: [],
    ...
    async GetDataCustomerIgnorePerPage(Page, PerPage, Search, StartDate, EndDate) {
        var _this = this;
        ...
        //console.log("GetDataCustomerIgnorePerPage");
        await axios.post(`${APIUrl}Customer/GetDataCustomerIgnorePerPage`, temp)
            .then(response => {
                if (response.data.status === 0) {
                    this.setState({ CustomerData: response.data.data });
                    // console.log(response.data.data);
                    let all_page = Math.ceil(parseInt(response.data.data[0].data_count) / parseInt(this.state.PerPage));
                    ...
                    response.data.data.map((item, index) => {
                        let TempSubData = {
                            ...
                            customer_status: item.status === 0 ? "รอดำเนินการ" :
                                item.status === 1 ? "ติดต่อสอบถาม" :
                                    item.status === 2 ? "รอชำระเงิน +อัพสลิป" :
                                        item.status === 3 ? "ตรวจสอบการชำระเงิน" :
                                            item.status === 4 ? "อนุมัติ +สร้างรหัสสมาชิก" :
                                                item.status === 5 ? <p style={{ color: 'red' }}> ลูกค้าไม่สนใจ </p> : "ไม่ระบุ",
                            decription: item.decription,
                            continue: item.status === 0 ? (this.state.Permission === "W" ? <button class="btn btn-info" style={{ marginLeft: '17%' }} type="button" onClick={() => this.SetIdAndStatus(item.id, item.status)} data-toggle="modal" data-target="#ConfirmPopup" data-backdrop="static" >รับเรื่อง</button> :
                                ...
                                : item.status === 1 ? (this.state.Permission === "W" ? <button class="btn btn-info" style={{ marginLeft: '17%' }} type="button" onClick={() => this.SetIdAndStatus(item.id, item.status)} data-toggle="modal" data-target="#ContactPopup" data-backdrop="static" >จัดการ</button> :
                                    ...
                                    : item.status === 2 ? (this.state.Permission === "W" ? <button class="btn btn-info" style={{ marginLeft: '17%' }} type="button" onClick={() => this.SetIdAndStatus(item.id, item.status)} data-toggle="modal" data-target="#AddSlipPopup" data-backdrop="static" >อัพสลิป</button> :
                                        ...
                                        : item.status === 3 ? (this.state.Permission === "W" ? <button class="btn btn-info" style={{ marginLeft: '17%' }} type="button" onClick={() => this.SetIdAndStatus(item.id, item.status)} data-toggle="modal" data-target="#ConfirmPopup" data-backdrop="static" >อนุมัติ</button> :
                                            ...
                                            : (this.state.Permission === "W" ? <button class="btn btn-info" style={{ marginLeft: '17%' }} type="button">สิ้นสุด</button> : <button disabled class="btn btn-info" style={{ marginLeft: '17%' }} type="button">สิ้นสุด</button>),
    ...
    EnterSearch = event => {
        if (event.key === 'Enter') {
            ...
            this.GetDataCustomerIgnorePerPage(1, this.state.PerPage, this.state.Search, this.state.StartDateSearch, this.state.EndDateSearch);
        }
    }
    ...
    ChangePerPage = event => {
        ...
        this.GetDataCustomerIgnorePerPage(this.state.Page, event.target.value, this.state.Search, this.state.StartDateSearch, this.state.EndDateSearch);
    };
    ...
    onPaginationNext() {
        ...
        this.GetDataCustomerIgnorePerPage(page, this.state.PerPage, this.state.Search, this.state.StartDateSearch, this.state.EndDateSearch);
        this.forceUpdate();
    };
    ...
    async ExportExcel(Search, StartDate, EndDate) {
        ...
        await axios.post(`${APIUrl}Customer/IgnoreExcel`, temp,
            {
                responseType: 'blob',
            }).then(response => {
                if (response.data != null) {
                    ...
                    link.href = url
                    link.setAttribute('download', moment(Date()).format("yyyyMMDDHHmm") + "_DataMembershipIgnore.xlsx")
                                ...
                                <div class="row align-items-center justify-content-between">
                                    <div class="col-auto mt-4">
                                        <h1 class="page-header-title">
                                            ...
                                            Manage Membership Ignore
                                        </h1>
                                        <div class="page-header-subtitle">จัดการข้อมูลการสมัครสมาชิกที่ไม่สนใจ</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <MDBCardBody>
                                        <div className="row">
                                            ...
                                            <div className="row">
                                                ...
                                                <button className="btn btn-success" onClick={() => this.GetDataCustomerIgnorePerPage(1, this.state.PerPage, this.state.Search, this.state.StartDateSearch, this.state.EndDateSearch)}>ค้นหา</button>
                                                        &nbsp;
                                                    </div>
                                                    <div className="col-md-5">
                            ...
                            <div className="AddModal-dialog">
                                <div className="modal-content" style={{ marginLeft: '30%', width: '50%', marginTop: '15%' }}>
                                    ...
                                    <div className="modal-body">
                                        <div className="row">
                                            <div className="form-group col-md6">
                                                <img class="table-img" src={`${APIImagePath}` + this.state.img_slip} />
                                            </div>
                                        </div>
                                        {/* <div className="row">
                                        <div className="form-group col-md6">
